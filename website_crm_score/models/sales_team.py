...
class team_user(models.Model):
    ...
    team_id = fields.Many2one('crm.case.section', string='SaleTeam', required=True, oldname='section_id')
...
class crm_team(osv.osv):
    ...
    def _get_default_team_id(self, user_id=None):
        team_id = super(crm_team, self)._get_default_section_id(user_id=user_id)
        if user_id is None:
    ...
    def _unassigned_leads(self):
        self.unassigned_leads = self.env['crm.lead'].search_count(
            [('section_id', '=', self.id), ('user_id', '=', False), ('assign_date', '=', False)]
        )
